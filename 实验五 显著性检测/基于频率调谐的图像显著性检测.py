...
def frequency_tuned_saliency():
    # 图像路径
    image_path = '../resources/assets/exp5/505.png'
    # 读取图像
    image = cv2.imread(image_path)
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

    # 步骤（1）：输入图像进行高斯滤波，去除高频信息
    gaussian_blur_image = cv2.GaussianBlur(image_rgb, (3, 3), 0)
    ...
    I_mu = np.mean(lab_image, axis=(0, 1))

    # 创建平均颜色图像
    mean_color_image = np.ones_like(lab_image) * I_mu[None, None, :]
    ...
    saliency_map = np.linalg.norm(lab_blur_image - mean_color_image, axis=2)
